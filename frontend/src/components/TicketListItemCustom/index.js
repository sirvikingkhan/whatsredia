...
const TicketListItemCustom = ({ ticket }) => {
  ...
  const handleSendMessage = async (id) => {
    ...
    const msg = `{{ms}} *{{name}}*, mi nombre es *${user?.name}* y ahora continuaré con su Servicio!`;
    const message = {
      read: 1,
      fromMe: true,
      mediaUrl: "",
          ...
          secondary={
            <span className={classes.contactNameWrapper}>
              ...
              <Typography
                ...
                variant="body2"
                color="textSecondary"
              > {ticket.lastMessage.includes('data:image/png;base64') ? <MarkdownWrapper> Ubicacion</MarkdownWrapper> : <MarkdownWrapper>{ticket.lastMessage}</MarkdownWrapper>}
