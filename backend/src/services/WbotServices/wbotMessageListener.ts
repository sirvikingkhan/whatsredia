...
const downloadMedia = async (msg: proto.IWebMessageInfo) => {
  ...
  while (contDownload < 10 && !stream) {
    try {
      ...
      logger.warn(
        `>>>> error ${contDownload} de descagar archivo ${msg?.key.id}`
      );
    }
  }
...
const handleOpenAi = async (
  ...
  const promptSystem = `Nas respostas utilize o nome ${sanitizeName(
    ...
    prompt.maxTokens
  } tokens e cuide para não truncar o final.\nSempre que possível, mencione o nome dele para ser mais personalizado o Servicio e mais educado. Quando a resposta requer uma transferência para o setor de Servicio, comece sua resposta com 'Ação: Transferir para o setor de Servicio'.\n
  ${prompt.prompt}\n`;
  ...
  if (msg.message?.conversation || msg.message?.extendedTextMessage?.text) {
    ...
    if (response?.includes("Ação: Transferir para o setor de Servicio")) {
      await transferQueue(prompt.queueId, ticket, contact);
      response = response
        .replace("Ação: Transferir para o setor de Servicio", "")
        .trim();
    }
  ...
  try {
    ...
    try {
      if (!msg.key.fromMe) {
        ...
        //  if ((ticket?.lastMessage.includes('_Insatisfeito_') || ticket?.lastMessage.includes('Por favor avalie nosso Servicio.')) &&  (!isFinite(rate_))) {
        ...
        //            }`,
        //            {
        //              text: 'Por favor avalie nosso Servicio.'
        //            }
        //          );
        //        },
        //        1000,
