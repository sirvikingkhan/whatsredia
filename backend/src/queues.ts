...
async function handleVerifyQueue(job) {
  logger.info("Buscando Servicios perdidos en las areas");
  try {
            ...
            if (
              ...
              if (count > 0) {
                tickets.map(async ticket => {
                  ...
                  logger.info(
                    `Asistencia perdida: ${ticket.id} - Empresa: ${companyId}`
                  );
                });
              } else {
                logger.info(
                  `No se encontró ningún servicio perdido - Empresa: ${companyId}`
                );
              }
            } else {
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let i = 0; i < contactData.length; i++) {
          ...
          logger.info(
            `Registro enviado a la cola de activación: Campanha=${campaign.id};Contato=${contacts[i].name};delay=${delay}`
          );
        }
...
async function handleDispatchCampaign(job) {
  try {
    ...
    logger.info(
      `Campaña enviada para: Campanha=${campaignId};Contato=${campaignShipping.contact.name}`
    );
  } catch (err: any) {
    Sentry.captureException(err);
...
export async function startQueueProcess() {
  logger.info("Iniciando procesamiento de areas");
